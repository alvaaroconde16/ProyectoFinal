<template>
  <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
    <div class="bg-white p-8 rounded-lg shadow-lg w-96 relative">
      ...
      <h2 class="text-2xl font-semibold mb-6 text-center text-gray-900">Iniciar Sesión</h2>
      ...
      <form @submit.prevent="handleLogin">
        ...
        <p class="text-sm text-gray-600 mt-4 text-center">
          ¿No tienes cuenta? <router-link to="/register" class="text-blue-500 hover:underline">Regístrate</router-link>
        </p>
...
<script setup>
  ...
  const emit = defineEmits(['closeModal', 'loginSuccess']); // Emitimos eventos al padre
